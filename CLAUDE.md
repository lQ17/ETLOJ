...
所有构建在本地完成，仅对本次实际发布范围涉及的组件执行对应测试、构建和打包；未变化且不受影响的组件无需重复测试或构建：
...
**默认只验证本次更新的功能及其直接受影响的链路，不执行无关功能的全站回归。** 发布范围必须依据线上实际版本与目标提交的差异确定，包含尚未上线的历史提交；不能只看目标提交的最后一次 diff。用户明确要求额外验收时，按用户指定范围执行。

- 发布前列出本次变更、受影响组件和对应验收项。没有变更或依赖影响证据的功能，无需验证；不能仅因 Server 重启、前端重新构建或 Prisma Client 重新生成，就扩大为全站验收。
- 保留最小部署健康检查：确认本次更新或重启的服务正常、对应入口或接口可用，并检查这些服务启动后的错误日志。发布前基线记录、备份、哈希校验及停机时的队列保护仍按发布顺序执行。
- 前端发布检查本次页面及其静态资源；涉及数据库变更或验收写入时，核对相关数据数量，解释变化并按精确范围清理临时数据。
- 仅修改题单分类等业务功能时，验证该功能的接口、页面交互和涉及的权限；无需重复测试登录流程、正式提交、运行代码、WebSocket、go-judge 或其他无关模块。
- 仅当提交、运行代码、Judge 消费或回调链路发生变化时，执行连续两次正式提交并穿插一次运行代码，检查相关队列是否存在异常残留。修改消费循环时，还须遵守“提交与判题”章节的专项测试要求。
- 仅当 Redis 消费连接逻辑发生变化时，验证正式队列 `brpoplpush` 与运行队列 `brpop` 使用两个独立阻塞连接。
- 仅当 WebSocket、ticket 鉴权、订阅逻辑或相关代理路由发生变化时，验证无 ticket 被拒绝、合法订阅收到结果及 HTTP 轮询兜底。
- 仅当 go-judge 启动参数、沙箱调用或相关网络配置发生变化时，检查执行冒烟、固定版本、监听地址及公网端口隔离；涉及 Nginx/HTTPS 配置时检查对应路由和配置有效性。
- 优先批量执行接口和数据断言，必要的交互通过 UI 验证；避免逐项手工创建大量测试数据和反复读取整页。已经通过的检查无需重复，除非后续变更或失败提供了重测理由。
- 交付报告区分已验证项、范围外未验证项和遗留问题，不得把未执行的检查写成通过。

### 回滚

回滚前先停止对应消费者，保存当前 Redis 队列快照；如果 processing 中有任务，必须先安全恢复到待判队列。然后恢复上一版目录、unit/Nginx 配置并启动服务，最后按上述范围规则执行最小部署健康检查，并复验导致回滚的功能及受回滚直接影响的链路，无需执行无关功能的全站回归。不要通过清空 Redis 或覆盖数据目录来回滚。
