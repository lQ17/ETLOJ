...
const SOURCE_CODE = `int upperBound(int arr[], int n, int target) {
...
function generateSteps(input: number[], target: number = 0): VisualStep[] {
  ...
  if (left < n) {
    steps.push({
      ...
      message: `upper_bound(${target}) = ${left}, arr[${left}]=${arr[left]}`,
      line: 10,
...
const upperBound: AlgorithmDef = {
  id: "upper-bound",
  name: "upper_bound",
  category: "searching",
  description: "查找有序数组中第一个大于 target 的位置。",
