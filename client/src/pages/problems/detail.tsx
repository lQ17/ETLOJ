...
import {
  Typography, Select, Button, Space, Tag, Spin, Message, Card, Input,
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <Button
      ...
      onClick={handleCopy}
      style={{ color: copied ? "rgb(var(--success-6))" : "var(--color-text-3)" }}
    />
  );
}
...
function parseSamples(md: string): { input: string; output: string }[] {
  ...
  const inputRegex = /###\s*输入\s*#\d+\s*\n\n```(?:\w*\n)?([\s\S]*?)```/g;
  const outputRegex = /###\s*输出\s*#\d+\s*\n\n```(?:\w*\n)?([\s\S]*?)```/g;
    ...
    <div style={{ display: "flex", gap: 24, height: "calc(100vh - 140px)", fontSize: 16 }}>
      ...
      <div style={{ flex: "0 0 60%", overflow: "auto", paddingRight: 8 }}>
        <Typography.Title heading={4}>
          <span style={{ fontFamily: "monospace", marginRight: 8, color: "var(--color-text-3)" }}>
            {problem.slug}
          </span>
          {problem.title}
        </Typography.Title>
        ...
        {samples.length > 0 && (
          <div style={{ marginTop: 24 }}>
            <Typography.Title heading={6} style={{ marginBottom: 12 }}>样例</Typography.Title>
            {samples.map((s, i) => (
              <div
                key={i}
                style={{
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        {/* 输入输出测试区 */}
        <div style={{ display: "flex", gap: 12, marginTop: 12, flex: 1, minHeight: 120 }}>
